...
Discovering associations between variables is *common*. Uncovering causality is *uncommon*. We don't automatically get causality by fitting and evaluating a statistical model. At a minimum, we need to come up with a story that realistically describes how our data was created.
...
The intuition is that our model can't determine how to accurately determine how different explanatory variables contribute to the story if they *make very similar contributions*.
...
- Have integrity in your analyses
...
Multiple regression is often referred to as "key drivers analysis" in business applications. Kroger has requested a key drivers analysis for the `WEST CENSUS TA` for sales of `CAMPBELL's` products. In the Nielsen `soup_data`, available on Canvas, the outcome of interest is `Sales`.
